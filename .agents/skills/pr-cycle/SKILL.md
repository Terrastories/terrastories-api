...
7. Verify the PR is merged and obtain an authoritative PR-to-result mapping from platform PR metadata, merge-event data, or an equivalent source that uniquely identifies the resulting commit for this PR. Refresh the target branch and prove that mapped result commit is reachable from the current target-branch tip. If no authoritative result mapping is available, do not infer it from the current target tip; stop in **unknown state** until the PR-to-result relationship can be proven.

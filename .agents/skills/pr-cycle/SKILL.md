...
- When a PR changes this notification contract, Hermes invocation, destination resolution, or profile integration, validate the real path before merge with one harmless direct send from the actual execution host using the explicit `terrastories` profile. Treat only Hermes exit code 0 as success. Capture or suppress raw Hermes stdout/stderr so transport output cannot expose recipient identifiers or secrets in chat, CI, or status logs; report only sanitized delivery status, and redact diagnostics before surfacing them if a failure needs investigation. Command construction, `--help`, or profile discovery alone is not delivery evidence.

...
import { z } from 'zod';
import { createStorySchema, updateStorySchema } from '../../db/schema/stories.js';

// Cultural protocols validation schema
export const CulturalProtocolsSchema = z.object({
  permissionLevel: z.enum(['public', 'community', 'restricted', 'elder_only']),
...
export const StoryCreateInputSchema = createStorySchema.extend({
...
// Story update schema
export const StoryUpdateInputSchema = StoryCreateInputSchema.partial().omit({
...
export const StoryFiltersSchema = z.object({
  ...
  {
    message: 'nearPoint and radiusKm must be provided together for geographic filtering',
  }
).refine(
  (data) => {
...
}).merge(StoryFiltersSchema.omit({ search: true })).strict();
...
export const StoryPlaceAssociationSchema = z.object({
...
export const StoryPlaceAssociationsSchema = z.object({
  associations: z.array(StoryPlaceAssociationSchema).max(20),
}).strict();

export const StorySpeakerAssociationSchema = z.object({
  speakerId: z.number().int().positive(),
  culturalRole: z.string().max(200).optional(),
  storyRole: z.enum(['narrator', 'subject', 'witness', 'elder_keeper']).default('narrator'),
  sortOrder: z.number().int().nonnegative().optional(),
}).strict();

export const StorySpeakerAssociationsSchema = z.object({
...
export const PaginatedStoriesResponseSchema = z.object({
...
export const CulturalProtocolErrorSchema = z.object({
  error: z.literal('cultural_protocol_violation'),
  message: z.string(),
  culturalGuidance: z.string().optional(),
}).strict();

export const DataSovereigntyErrorSchema = z.object({
  error: z.literal('data_sovereignty_violation'),
  message: z.string(),
}).strict();

export const InsufficientPermissionsErrorSchema = z.object({
...
export const CreateStorySuccessSchema = z.object({
  success: z.literal(true),
  data: StoryWithRelationsResponseSchema,
  message: z.string(),
}).strict();

export const UpdateStorySuccessSchema = z.object({
  success: z.literal(true),
  data: StoryWithRelationsResponseSchema,
  message: z.string(),
}).strict();

export const DeleteStorySuccessSchema = z.object({
  success: z.literal(true),
  message: z.string(),
}).strict();

export const ListStoriesSuccessSchema = z.object({
  success: z.literal(true),
...
export const validateGeographicFilter = (filter: { nearPoint?: unknown; radiusKm?: unknown }) => {
...
export type StoryWithRelationsResponse = z.infer<typeof StoryWithRelationsResponseSchema>;
export type PaginatedStoriesResponse = z.infer<typeof PaginatedStoriesResponseSchema>;
...
export type StorySpeakerAssociation = z.infer<typeof StorySpeakerAssociationSchema>;
